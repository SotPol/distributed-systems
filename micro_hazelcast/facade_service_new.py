...
CONFIG_SERVER_URL = httplocalhost6000services
...
def route_message()
    ...
    log_url = fhttp{instance['host']}{instance['port']}logs
...
def aggregate_data()
    ...
    try
        log_data = requests.get(fhttp{instance_log['host']}{instance_log['port']}logs).json()
        msg_data = requests.get(fhttp{instance_msg['host']}{instance_msg['port']}messages).json()
    except Exception as e
